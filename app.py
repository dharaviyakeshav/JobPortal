...
app = Flask(__name__)
app.secret_key = os.getenv("SECRET_KEY", "dev-only-change-this-on-render")
...
def send_email(to_email, subject, body):
    ...
    try:
        ...
        print("EMAIL AUTH ERROR: Check Gmail App Password and 2-Step Verification.", e)
        return False
...
def send_otp_email(email, otp):
    return send_email(email, "OTP Verification - Job Portal", f"Your OTP for verification is: {otp}\n\nPlease enter this OTP to complete registration.")
...
#------------verify otp------------------
# OTP verification is handled by the /register route above.
...
Job Portal Team
"""
    return send_email(email, "Job Application Successful", body)
...
Job Portal Team
"""
    return send_email(company_email, "New Job Application Received", body)
...
{message}
"""
        admin_email = os.getenv("ADMIN_EMAIL") or os.getenv("GMAIL_EMAIL")
        if admin_email:
            send_email(admin_email, f"New Contact: {subject}", admin_body)
        user_body = f"""Hi {name},
...
"""
        send_email(email, "We received your request - Job Portal", user_body)
